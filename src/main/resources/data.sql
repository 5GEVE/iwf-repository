...
VALUES ('provider', false, true, false, (SELECT id FROM site WHERE name='SPAIN_5TONIC'), '10.5.7.0/24', '172.17.254.0/24')
ON CONFLICT (vim_network_name, site_id) DO NOTHING;
INSERT INTO network (vim_network_name, floating_ip, mgmt_net, external_net, site_id, cidr, ip_mapping)
VALUES ('provider2', false, false, true, (SELECT id FROM site WHERE name='SPAIN_5TONIC'), '10.5.7.5/24', '172.17.254.0/24')
ON CONFLICT (vim_network_name, site_id) DO NOTHING;
